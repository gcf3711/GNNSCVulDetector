...
contract RenaswapV1Router is IRenaswapV1Router {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IUniswapV2Pair(RenaswapV1Library.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, RenaswapV1Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            IUniswapV2Pair(pair).token1(), msg.sender, pair, amountIn
        );
        ...
        require(
            ...
            'RenaswapV1Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        require(path[path.length - 1] == WETH, 'RenaswapV1Router: INVALID_PATH');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, RenaswapV1Library.pairFor(factory, path[1], path[0]), amountIn
        );
...
library RenaswapV1Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'c346c428cf4a1855040b9f552358dc5437d9879f85fea3b4a793682741bcec03'  
            ))));
    }
