...
contract SushiswapIntegration is LiquidityNexusBase {
    ...
    address public constant SLP = address(0x397FF1542f962076d0BFE58eA045FfA2d347ACa0);  
    address public constant ROUTER = address(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F);  
    ...
    function _poolSwapExactUSDCForETH(uint256 inUSDC) internal returns (uint256 outETH) {
        ...
        uint256[] memory amounts =
            IUniswapV2Router02(ROUTER).swapExactTokensForTokens(inUSDC, 0, pathToETH, address(this), block.timestamp);  
...
contract NexusLPSushi is ERC20("Nexus LP SushiSwap ETH/USDC", "NSLP"), RebalancingStrategy1 {
        ...
        )
    {
        ...
        (pairedUSDC, pairedETH, liquidity) = _poolAddLiquidityAndStake(amountETH, block.timestamp);  
    ...
    function emergencyExit(address[] memory minterAddresses) external onlyOwner {
        for (uint256 i = 0; i < minterAddresses.length; i++) {
            ...
            if (shares > 0) {
                _unpair(minterAddress, minter, shares, block.timestamp);  
            }
        }
