...
contract LockHandler {
 ...
 ,
...

    {
...

      {
        ...
        if (coverAsset == ETH) {
          ...
          (bool ok,  ) = address(pool).call.value(premiumAmount)("");
  ...
  {
    bytes4 currency;
    ( , memberAddress, contractAddress, currency,  , premiumInNXM) = quotationData.getCoverDetailsByCoverID1(coverId);
    ( , status, sumAssured, coverPeriod, validUntil) = quotationData.getCoverDetailsByCoverID2(coverId);
  ...
  function executeCoverAction(uint  , uint8  , bytes calldata  )
  external
  payable
